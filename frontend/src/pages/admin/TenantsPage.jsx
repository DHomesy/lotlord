...
export default function TenantsPage() {
  ...
  const now = new Date()
  // Only show tenant-type invitations on this page; employee invites live in /team
  const tenantInvites  = allInvites.filter((r) => r.type !== 'employee')
  const pendingInvites = tenantInvites.filter((r) => !r.accepted_at && new Date(r.expires_at) > now)
  const acceptedInvites = tenantInvites.filter((r) => !!r.accepted_at)
  const expiredInvites  = tenantInvites.filter((r) => !r.accepted_at && new Date(r.expires_at) <= now)
  ...
  return (
    <PageContainer
      title="Tenants"
      actions={<Button variant="contained" startIcon={<AddIcon />} onClick={() => setOpen(true)}>Invite Tenant</Button>}
    >
      <>
