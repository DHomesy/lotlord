...
router.post('/batch', authenticate, authorize('admin', 'landlord'), createChargesBatchValidators, validate, controller.createChargesBatch);

// POST /api/v1/charges/void-by-unit — void all unpaid charges for a unit (bulk replace)
router.post('/void-by-unit', authenticate, authorize('admin', 'landlord'), voidChargesByUnitValidators, validate, controller.voidChargesByUnit);
